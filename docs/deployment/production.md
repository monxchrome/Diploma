...
2. Create external Docker secrets named in `docker-compose.prod.yml` (`jwt_access_secret`, `refresh_token_pepper`, `ingestion_internal_secret`, MinIO credentials, and Stripe keys when billing is enabled).
