...
const pool = new Pool({
  connectionString:
    ...
    "postgresql://dip_user:dip_password@localhost:5432/dip?schema=public",
});
...
async function main(): Promise<void> {
  const project = process.env.BENCHMARK_DEMO_PROJECT_ID
    ? await prisma.project.findUniqueOrThrow({
        where: { id: process.env.BENCHMARK_DEMO_PROJECT_ID },
      })
  ...
  const profile = await prisma.modelProfile.findFirstOrThrow({
    where: { code: "qwen-ollama-default", active: true, benchmarkEligible: true },
  });
  const prompt = await prisma.promptVersion.findUniqueOrThrow({
    where: { role_version: { role: "SINGLE_AGENT", version: "phase-11-v1" } },
  });
  const rubric = await prisma.evaluationRubric.findUniqueOrThrow({
  ...
  const suite = await prisma.benchmarkSuite.upsert({
    ...
    create: {
      ...
      code: "phase-11-demo",
      version: "v1",
      title: "Phase 11 local Qwen demo",
      description: "One-case controlled-evidence demonstration run.",
      researchQuestions: json(["How does the local Qwen baseline handle a bounded decision case?"]),
      hypotheses: json(["A pinned local model produces a structurally valid decision artifact."]),
  ...
  const variant = await prisma.benchmarkVariant.upsert({
    ...
    create: {
      ...
      description: "Pinned local Qwen single-agent baseline.",
      architecture: "SINGLE_AGENT",
      budgetConfiguration: json({ maxOutputTokens: 256 }),
      contentHash: "phase-11-demo-v5",
    },
  ...
  await prisma.benchmarkVariant.update({
    where: { id: variant.id },
    data: { frozenAt: new Date() },
  });
  await prisma.benchmarkSuite.update({
    where: { id: suite.id },
    data: { status: "READY", frozenAt: new Date() },
  });
  const testCase = datasetVersion.cases[0];
  if (!testCase) throw new Error("Synthetic benchmark case not found");
  const evidence = testCase.evidencePackages.find(
    (item) => item.protocol === "CONTROLLED_EVIDENCE",
  );
  ...
  const run =
    ...
    (await prisma.benchmarkRun.create({
      data: {
        ...
        budgetProtocol: "PRODUCTION_DEFAULT_BUDGET",
        repetitions: 1,
        randomizationSeed: 11042,
        status: "QUEUED",
        startedById: user.id,
        idempotencyKey: randomUUID(),
        executionPlan: json({
          order: [{ caseId: testCase.id, variantId: variant.id, repetitionIndex: 1 }],
        }),
        evaluationPolicy: json({ humanEvaluation: false }),
        currency: "USD",
        caseRuns: {
          create: {
  ...
  await queue.add(
    "execute",
    { benchmarkRunId: run.id, requestId: `demo-${run.id}` },
    { jobId: `benchmark-run:${run.id}-${Date.now()}` },
  );
