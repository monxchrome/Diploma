    ...
    <section className="grid gap-4 rounded-md border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Turn project evidence into a traceable decision brief with specialist agents.
          </p>
        </div>
      ...
      {!analyses.isLoading && !analyses.isError && analyses.data?.length === 0 ? (
        <div className="rounded-md border border-dashed border-slate-300 bg-slate-50 p-6 text-center">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Start with a decision question and the evidence you want to ground it in.
          </p>
        </div>
      ) : null}
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-wrap gap-2 text-xs text-slate-500">
                <span>{analysis.mode === "MULTI_AGENT" ? "Multi-agent" : "Single-agent"}</span>
                <span>·</span>
